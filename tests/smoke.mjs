...
const fixtures = JSON.parse(
  await readFile(
    new URL("../docs/sample-responses.json", import.meta.url),
    "utf8",
  ),
);
const sample = (method, route) =>
  structuredClone(
    fixtures.endpoints.find(
      (entry) => entry.method === method && entry.path === route,
    ).sampleResponse,
  );
...
const backend = createServer(
  { key: certificates.private, cert: certificates.cert },
  async (request, response) => {
    const route = new URL(request.url, "https://localhost").pathname.replace(
      "/api/v1",
      "",
    );
    const role = request.headers.cookie?.includes("access_token=staff")
      ? "ADMIN"
      : request.headers.cookie?.includes("access_token=patient")
        ? "PATIENT"
        : null;
    ...
    if (route === "/auth/login") {
      ...
      response.setHeader("Set-Cookie", [
        "access_token=staff; HttpOnly; Secure; Path=/api/v1",
        "refresh_token=test; HttpOnly; Secure; Path=/api/v1/auth",
      ]);
    ...
    if (route === "/auth/patient/verify") {
      response.setHeader(
        "Set-Cookie",
        "access_token=patient; HttpOnly; Secure; Path=/api/v1",
      );
      return send(sample("POST", route));
    }
    if (route === "/auth/logout") {
      response.setHeader("Set-Cookie", [
        "access_token=; Max-Age=0; Path=/api/v1",
        "refresh_token=; Max-Age=0; Path=/api/v1/auth",
      ]);
      return send(sample("POST", route));
    }
    if (route === "/auth/refresh") {
      if (!request.headers.cookie?.includes("refresh_token=test"))
        return send({ success: false }, 401);
      refreshes++;
      response.setHeader(
        "Set-Cookie",
    ...
    if (route === "/auth/me") {
      const identity = sample("GET", route);
      identity.data.user.role = role;
      return send(identity);
    }
    if (route === "/patients/me")
      return registered
        ? send(sample("GET", route))
        : send({ success: false }, 404);
    if (route === "/patients" && request.method === "POST") {
      registered = true;
      return send(sample("POST", route), 201);
    }
    if (route === "/doctors") {
      const query = new URL(request.url, "https://localhost").searchParams;
      if ([...query.keys()].some((key) => key !== "active"))
        return send({ success: false }, 400);
    ...
    if (/^\/files\/[^/]+\/url$/.test(route))
      return send(sample("GET", "/files/:id/url"));
    ...
    if (route === "/appointments/stats")
      return role === "ADMIN"
        ? send(sample("GET", route))
        : send({ success: false }, 403);
    if (route === "/appointments" && request.method === "GET")
      return role === "ADMIN"
        ? send(sample("GET", route))
        : send({ success: false }, 403);
    if (route === "/appointments" && request.method === "POST")
      return send(sample("POST", route), 201);
    if (/^\/appointments\/[^/]+$/.test(route))
      return send(sample("GET", "/appointments/:id"));
    if (/^\/appointments\/[^/]+\/(schedule|cancel)$/.test(route))
      return role === "ADMIN"
        ? send(sample("PATCH", `/appointments/:id/${route.split("/").at(-1)}`))
        : send({ success: false }, 403);
...
const frontend = spawn(
  ...
  ],
  {
    ...
    env: {
      ...
      API_BASE_URL: "https://localhost:4443/api/v1",
...
try {
  ...
  for (let attempt = 0; attempt < 60; attempt++) {
    try {
      if ((await fetch("http://localhost:3100/login")).ok) {
        ready = true;
        break;
      }
    } catch {}
  ...
  page.on("console", (message) => {
    if (
      message.type() === "error" &&
      /Content Security Policy|Hydration|hydration/.test(message.text())
    )
  ...
  assert.equal(
    cookies.find((cookie) => cookie.name === "refresh_token")?.path,
    "/api/backend/auth",
  );
  await page.context().clearCookies({ name: "access_token" });
  const otherTab = await page.context().newPage();
  await Promise.all([
    page.goto("http://localhost:3100/session"),
    otherTab.goto("http://localhost:3100/session"),
  ]);
  await Promise.all([
    page.waitForURL("**/admin"),
    otherTab.waitForURL("**/admin"),
  ]);
  assert.equal(
    refreshes,
    1,
    "Two tabs must not rotate the same refresh token twice",
  );
  ...
  await page.getByLabel("Phone Number", { exact: true }).fill("+2348012345678");
  ...
  await page
    .getByLabel("Emergency contact name", { exact: true })
    .fill("Example Contact");
  await page
    .getByLabel("Emergency contact number", { exact: true })
    .fill("+2348012345678");
  await page
    .getByLabel("Insurance provider", { exact: true })
    .fill("Example Insurance");
  await page
    .getByLabel("Insurance policy number", { exact: true })
    .fill("POLICY-123");
  for (const checkbox of await page.getByRole("checkbox").all())
    await checkbox.check();
  await page.getByRole("button", { name: "Submit and Continue" }).click();
  await page.waitForURL("**/new-appointment");
  await capture("appointment");
  ...
  await page
    .getByText("Your selected doctor", { exact: true })
    .or(page.getByText("Dr. Ada Okafor", { exact: true }))
    .waitFor();
  ...
  await page
    .getByRole("link", { name: "Download document (temporary link)" })
    .click();
  assert.equal(await (await downloadReady).failure(), null);
  await page.goto("http://localhost:3100/admin");
  await page
    .getByRole("heading", { name: "Let’s get you back on track.", exact: true })
    .waitFor();
  ...
  await page.goto(
    "http://localhost:3100/reset-password#token=test-fragment-token",
  );
  await page.getByLabel("Token from your email").waitFor();
  await expect(page.getByLabel("Token from your email")).toHaveValue(
    "test-fragment-token",
    { timeout: 20_000 },
  );
  ...
  console.log(
    `${mode} browser smoke checks passed: CSP, staff login/dashboard/cancellation/logout, two-tab refresh, unauthenticated redirect, patient OTP/registration/booking, document upload/download, patient/admin isolation, mobile layout, reset token links.`,
  );
} finally {
  await browser?.close();
  frontend.kill();
