...
export default function RegisterForm({ user }: { user: User }) {
  ...
  const field = (
    name: keyof z.infer<typeof PatientFormValidation>,
    label: string,
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(submit)} className="flex-1 space-y-12">
        <section className="space-y-3">
          <p className="eyebrow">PATIENT PROFILE</p>
          <h1 className="header">Let’s get to know you.</h1>
          <p className="text-muted-foreground">
            Help your care team prepare. Complete the details below; optional
            fields are marked.
          </p>
        </section>
        ...
        <section className="form-section space-y-6">
          ...
          <div className="flex flex-col gap-6 md:flex-row">
            ...
            {field("insurancePolicyNumber", "Insurance policy number")}
          </div>
          <div className="flex flex-col gap-6 md:flex-row">
            {field("allergies", "Allergies (optional)", FormFieldType.TEXTAREA)}
            {field(
              "currentMedication",
        ...
        <section className="form-section space-y-6">
          ...
          {field(
            "privacyConsent",
            "I consent to the processing of my information for patient care.",
            FormFieldType.CHECKBOX,
          )}
        </section>
        {error && (
